...
if ! python3 -c "import rich" &> /dev/null || ! command -v gum &> /dev/null; then
    echo "Descargando motor gráfico de terminal (python-rich y gum)..."
    sudo pacman -Sy --noconfirm python-rich gum >/dev/null 2>&1
fi
