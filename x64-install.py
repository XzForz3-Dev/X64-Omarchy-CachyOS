...
def update_ui():
    ...
    if current_state == "menu":
        ...
        MAX_VISIBLE = 7
        start_idx = max(0, item_idx - (MAX_VISIBLE // 2))
        ...
        if start_idx > 0:
            item_text += "  [dim cyan]... (↑ Más arriba)[/dim cyan]\n\n"
        ...
        for i_vis, item in enumerate(visible_items):
            ...
            if item["type"] == "action":
                ...
                item_text += f"{cursor}[{style}]{item['label']}[/{style}]\n\n"
            else:
                ...
                desc = item.get("desc", "")
                desc_style = "white" if is_active else "dim white"
                
                item_text += f"{cursor}{chk_box} [{style}]{item['label']}[/{style}]\n"
                item_text += f"           [{desc_style}]{desc}[/{desc_style}]\n\n"
                
        if end_idx < len(items):
            item_text += "  [dim cyan]... (↓ Más abajo)[/dim cyan]\n"
