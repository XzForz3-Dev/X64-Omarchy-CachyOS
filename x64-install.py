...
def installer_worker():
    ...
    try:
        ...
        run_cmd_live("sudo cp -r bin config default shell themes /usr/share/omarchy/", check=False)
        ...
        run_cmd_live("cp -r config/* ~/.config/", check=False)
        run_cmd_live("cp -r bin/* ~/.local/bin/", check=False)
        run_cmd_live("cp -r themes/* ~/.local/share/themes/", check=False)
