...
def setup_plymouth_bootloader():
    ...
    run_cmd_live("sudo bash -c 'grep -q \"COMPRESSION_OPTIONS=\\\"(-T0)\\\"\" /etc/mkinitcpio.conf || sed -i \"s/^#COMPRESSION_OPTIONS=.*/COMPRESSION_OPTIONS=\\\"(-T0)\\\"/\" /etc/mkinitcpio.conf' 2>/dev/null", check=False)
