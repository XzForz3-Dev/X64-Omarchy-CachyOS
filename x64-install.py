...
"""
        for item in menu_data[0]["items"]:
            ...
            echo " [$idx] GNOME"
            set -a options $idx
            set -a cmds "omarchy-gnome-session-mock &; exec env XDG_SESSION_TYPE=wayland GBM_BACKEND=nvidia-drm __GLX_VENDOR_LIBRARY_NAME=nvidia dbus-run-session gnome-shell --wayland"
            set idx (math $idx + 1)
        end\n"""
                elif "Niri" in label:
